...
QString MemoryEntry::localizedHelp(Language language) const
{
    if (m_totalRam.has_value() && m_availableRam.has_value()) {
        ...
        return localize(ki18nc("@info:tooltip, referring to system memory or RAM",
                               "Some memory is reserved for use by the kernel or system hardware such as integrated graphics memory. Memory can also be "
                               "unavailable due to hardware or firmware misconfiguration."),
                        language);
    }
    ...
    if (m_totalRam.has_value() && !m_availableRam.has_value()) {
        ...
        return localize(
            ki18nc("@info:tooltip, referring to system memory or RAM",
                   ...
                   "hardware, such as integrated graphics memory. Memory can also be unavailable due to hardware or firmware misconfiguration."),
            language);
    }
